...
func TestGetNotificationsChannelIDs(t *testing.T) {
	for name, val := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			monkey.Patch(GetSubscriptionsByURLSpaceKey, func(_, spaceKey string) (serializer.StringArrayMap, error) {
				return val.urlSpaceKeyCombinationSubscriptions, nil
			})
			monkey.Patch(GetSubscriptionsByURLPageID, func(_, pageID string) (serializer.StringArrayMap, error) {
